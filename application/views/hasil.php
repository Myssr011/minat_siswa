	...
	<div class="container wrapsearch" style="background-color: #fefefe; border: 1px solid #f5f5f5; min-height:100vh" >
        ...
        <table class="table table-bordered table-striped text-center">
          ...
          foreach($result as $key => $res){ 
            $cca_array = explode(",",$res['cca']);
            foreach ($cca_array as $val_cca){
            ?>
          <tr>
            <td><?= $no++;?></td>
            <td><?= $res['nis'];?></td>
            ...
            <td><?= $val_cca; ?></td>
            <td><?= $res['waktu'];?></td>
            <td><a href="<?=base_url('home/saveMinatBakat?nis='.$res['nis'].'&persen='.$res['persen'].'&hasil='.$res['hasil'].'&cca='.$val_cca.'&waktu='.$res['waktu'])?>" class="btn btn-info btn-sm">PILIH</a></td>
          </tr>
          <?php } } ?> 
        </table>
        <center>
          <a href="<?php echo base_url();?>" class="btn btn-success"><span class="fa fa-arrow-left"></span> Kembali</a>
          <br/><br/>
